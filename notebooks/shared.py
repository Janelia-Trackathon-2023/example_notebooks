...
# File copied from the cell tracking metrics package, which in turns copies much from ctc-tools
# Ideally, should be in a central IO format
...
def format_ctc_data(masks, track_path):
    """Read the CTC track file and create a Graph.
    ...
    names = ["Cell_ID", "Start", "End", "Parent_ID"]
    tracks = pd.read_csv(
        track_path, header=None, sep=" ", names=names
    )
    detections = get_node_attributes(masks)
    G = ctc_to_graph(tracks, detections)
